 ...
 */
static void
COMPILER_ATTR(format(printf, 3, 4)) catbuf_internal(char *buf, size_t len, const char *fmt, ...) {
    va_list argptr;
    va_start(argptr, fmt);
...
tss2_fmt1_err_strs_get(TSS2_RC error) {
    ...
    static const char *fmt1_err_strs[] = {
        ...
        "the TPM was unable to unmarshal a value because there were not enough octets in the input "
        "buffer",
...
tpm2_err_handler_fmt1(TPM2_RC rc) {
    ...
    const char *m = tpm2_rc_fmt1_P_get(rc) ? "parameter" : is_handle ? "handle" : "session";
    catbuf(buf, "%s", m);
...
static struct {
    ...
    ADD_HANDLER("tpm", tpm2_ehandler), ADD_NULL_HANDLER, /* layer 1  is unused */
 ...
 */
TSS2_RC_HANDLER
Tss2_RC_SetHandler(UINT8 layer, const char *name, TSS2_RC_HANDLER handler) {
 ...
 *     - Examples:
 *       - error(1.2): bad tag
 *       - warn(2.0): the 1st handle in the handle area references a transient object or session
 * that is not loaded
 *
