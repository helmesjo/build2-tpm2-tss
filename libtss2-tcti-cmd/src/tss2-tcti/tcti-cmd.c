...
#define close_fd(fd)                                                                               \
    do {                                                                                           \
        if (close(fd)) {                                                                           \
            LOG_WARNING("Could not close fd (%d): %s", fd, strerror(errno));                       \
        }                                                                                          \
    } while (0)

typedef enum child_behavior child_behavior;
enum child_behavior { child_behavior_good, child_behavior_bad, child_behavior_error };
...
TEST_VISIBILITY WEAK int
tcti_cmd_pipe(int pipefd[2]) {
    return pipe(pipefd);
}
...
TEST_VISIBILITY WEAK FILE *
tcti_cmd_fdopen(int fd, const char *mode) {
    return fdopen(fd, mode);
}

TEST_VISIBILITY WEAK int
tcti_cmd_sigprocmask(int how, const sigset_t *set, sigset_t *oldset) {
    return sigprocmask(how, set, oldset);
}

TEST_VISIBILITY WEAK size_t
tcti_cmd_fwrite(const void *ptr, size_t size, size_t nmemb, FILE *stream) {
    return fwrite(ptr, size, nmemb, stream);
}

TEST_VISIBILITY WEAK size_t
tcti_cmd_fread(void *ptr, size_t size, size_t nmemb, FILE *stream) {
    return fread(ptr, size, nmemb, stream);
}

TEST_VISIBILITY WEAK int
tcti_cmd_ferror(FILE *stream) {
...
reap_child(pid_t child_pid) {
    ...
    if (reaped > 0) {
        ...
        LOG_WARNING("Error getting wstatus of pid (%u): %s", reaped, strerror(errno));
    ...
    if (rc < 0) {
        /* can't kill it, why? */
        LOG_ERROR("Error shutting down pid (%u): %s", child_pid, strerror(errno));
        goto out;
    }
...
setup_child_and_exec(int stdin_pipefd[2], int stdout_pipefd[2], const char *cmd) {
    ...
    if (rc) {
        LOG_ERROR("Error prctl (PR_SET_PDEATHSIG, SIGTERM): %s", strerror(errno));
        goto error;
    }
    ...
    LOG_ERROR("Error execlp (\"/bin/sh\", \"sh\", \"-c\", \"%s\","
              " (char*) NULL): %s",
              cmd, strerror(errno));
 ...
 */
static int
popen_w_pipes(const char *cmd, pid_t *pid, FILE **sink, FILE **source) {
    ...
    *source = tcti_cmd_fdopen(stdout_pipefd[0], "rb");
    ...
    if (rc) {
        LOG_WARNING("Could not set stdin child pipe to non-buffering: %s", strerror(errno));
    }

    rc = setvbuf(*source, NULL, _IONBF, 0);
    if (rc) {
        LOG_WARNING("Could not set stdout child pipe to non-buffering: %s", strerror(errno));
    }
...
TSS2_RC
tcti_cmd_transmit(TSS2_TCTI_CONTEXT *tcti_ctx, size_t size, const uint8_t *cmd_buf) {
    ...
    TSS2_RC rc = tcti_common_transmit_checks(tcti_common, cmd_buf, TCTI_CMD_MAGIC);
...
TSS2_RC
tcti_cmd_get_poll_handles(TSS2_TCTI_CONTEXT     *tctiContext,
...
void
tcti_cmd_finalize(TSS2_TCTI_CONTEXT *tctiContext) {
...
TSS2_RC
tcti_cmd_receive(TSS2_TCTI_CONTEXT *tctiContext,
    ...
    rc = tcti_common_receive_checks(tcti_common, response_size, TCTI_CMD_MAGIC);
    if (rc != TSS2_RC_SUCCESS) {
        return rc;
    }
    ...
    if (size != TPM_HEADER_SIZE) {
        LOG_ERROR("Read was not size of header, got %zu expected %zu", size, TPM_HEADER_SIZE);
    ...
    size = tcti_cmd_fread(&response_buffer[TPM_HEADER_SIZE], 1, data_size, tcti_cmd->source);
    rc = tcti_cmd_ferror(tcti_cmd->source);
    if (rc) {
        rc = errno == EFAULT ? TSS2_TCTI_RC_INSUFFICIENT_BUFFER : TSS2_TCTI_RC_IO_ERROR;
        LOG_ERROR("Reading from command TCTI: %s", strerror(errno));
        goto out;
    }
...
void
tcti_cmd_init_context_data(TSS2_TCTI_COMMON_CONTEXT *tcti_common) {
 ...
 */
TSS2_RC
Tss2_Tcti_Cmd_Init(TSS2_TCTI_CONTEXT *tctiContext, size_t *size, const char *conf) {
    TSS2_TCTI_CMD_CONTEXT    *tcti_command = (TSS2_TCTI_CMD_CONTEXT *)tctiContext;
    ...
    int rc
        = popen_w_pipes(conf, &tcti_command->child_pid, &tcti_command->sink, &tcti_command->source);
    if (rc != 0) {
        LOG_ERROR("Open subprocess command \"%s\", failed with: %s", conf, strerror(rc));
